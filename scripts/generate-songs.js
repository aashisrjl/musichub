...
// Generate song entry
function generateSongEntry(filename, category, index, useLogo) {
  ...
  const artwork = useLogo ? "require('../public/logo.png')" : "require('../public/banner.jpeg')";
  ...
  categories.forEach((folder, folderIndex) => {
    ...
    if (files.length > 0) {
      ...
      files.forEach((file, index) => {
        // Alternate between logo and banner based on index
        const useLogo = (folderIndex + index) % 2 === 0;
        content += generateSongEntry(file, folder, index, useLogo) + ',\n';
      });
